...
do 
  case $option in
	\?)
	 echo "Usage:autobuild_ubuntu18.04.sh [-t {target}][-m {mode}] | -t {default;MND;MOIS}:custom site -m {static;repo}:mode " 1>&2 
	 exit 1
	 ;;
	t)
	 t_value=$OPTARG
...
if [[ ${m_value} == "static" ]];then 
 bash ./oneStepBuild_staticInstall.sh -t ${t_value} -o ubuntu18.04
else
 bash ./oneStepBuild_repoInstall.sh -t ${t_value} -o ubuntu18.04
fi
