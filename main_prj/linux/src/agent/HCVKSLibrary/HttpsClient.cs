...
namespace HCVK.HCVKSLibrary
{
    ...
    public class HttpsClient
    {
        ...
        {
            _logger.Info(string.Format("Query URL: {0}", strRequestUrl));
            _logger.Info(string.Format("Query Param: {0}", JsonConvert.SerializeObject(oJSonParam, Formatting.Indented)));
        ...
        public void SendReqeust_AsyncCallback(string strRequestUrl, string strMethod, Dictionary<string, string> dicHeaders, object oJSonParam, CallbackResponse callbackResponse)
        {
            try
            {
                _logger.Info(string.Format("Query URL: {0}", strRequestUrl));
